...
func _ready():
	...
	# So fire_resistance = 0.7 means actual protection = 30%.
	fire_resistance = 0.7
	wind_resistance = 1
	water_resistance = 1
	sturdiness = 1
	...
	for x in range(plot_index[0] - 1, plot_index[0] + 2):
		for y in range(plot_index[1] - 1, plot_index[1] + 2):
			...
			if _buffed_positions.has(pos):
				continue
			# Treat tile.fire_resistance as vulnerability (1.0 = full damage).
			var current = tile.get("fire_resistance")
			if current == null:
				# default missing tiles to full vulnerability
				current = 1.0
				tile.set("fire_resistance", current)
			# Apply +50% protection -> reduce vulnerability by FIRE_BUFF (clamped 0..1)
			var new_vuln = clamp(current - FIRE_BUFF, 0.0, 1.0)
