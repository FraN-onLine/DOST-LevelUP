...
func _ready():
	...
	if is_host:
		...
		# Prefill name with the host name we set on the Hosting screen
		name_input.text = Network.host_name
	else:
...
func _change_name():
	...
	var my_id = multiplayer.get_unique_id()
	if is_host:
		# Update host_name on Network so LAN discovery broadcasts the new name
		Network.set_host_name(new_name)
